...
Für eine erste Analyse, in welchem Rahmen sich die Berechnungszeiten und die erreichte Genauigkeit befinden, wird das zuvor diskutierte Beispiel in MATLAB implementiert und ausgewertet. Dabei wird noch keine Parallelisierung vorgenommen, die Resultate stammen von einem mit 2,7 GHz getaktetem i7, wobei nur ein Kern verwendet wird.

\begin{figure}[h]
    \centering
    \includegraphics[width=14cm]{images/Berechnungszeit.eps}
    \caption{Benötigte Zeit zur Abschätzung von $\pi$}
    \label{fig:Berechnungszeit}
\end{figure}

Bei sehr wenigen HM Ereignissen dominiert die Rechenzeit für Initialisierungen etc. Bereits ab ca. 100 Ereignissen ist eine direkte Proportionalität zwischen Rechenzeit und Anzahl Ereignissen erkennbar. Pro Sekunde werden rund $16,7 * 10^{6}$ Ereignisse berechnet.   

\begin{figure}[h]
    \centering
    \includegraphics[width=14cm]{images/Fehler.eps}
