...
{
    const double count = NUM_OF_IT;
    
    uint64_t result = 0;                   // results returned from device
    double pi;
    ...
    //
    output = clCreateBuffer(context, CL_MEM_READ_WRITE	, sizeof(uint64_t), NULL, NULL);
    if (!output)
    {
        printf("Error: Failed to allocate device memory!\n");
        exit(1);
    }
    clEnqueueWriteBuffer(commands, output, CL_TRUE, 0, sizeof(result), &result, 0, NULL, NULL);
    ...
    //
    err = clEnqueueReadBuffer( commands, output, CL_TRUE, 0, sizeof(uint64_t), &result, 0, NULL, NULL );
    if (err != CL_SUCCESS)
    {
    ...
    //
    pi = 4*((double)result) / ((double)count);
    printf("%lf\n", pi);
